...
* Procedure.
* <p> This class supersedes an older class called 'statementParameters.java', which has been retired.
...
public class StatementParameters2
{
  ...
  * @param int parameterId The id of the parameter to set. Id's start at 0.
  * @param Clob parameterValue An Clob containing CLOB Data.
  * @throws CSException if <tt>parameterId</tt> is not a valid parameter.
  */
  public void setParam(int parameterId
                      ,Clob parameterValue) throws CSException
    {
    checkRange(parameterId);
    parameterArray[parameterId-1] = parameterValue;
    inputParameterSetArray[parameterId-1] = true;
    parameterTypeArray[parameterId-1] = Types.CLOB;
    }
  ...
  * @param int parameterId The id of the parameter to set. Id's start at 0.
  * @param Blob parameterValue An Blob containing BLOB Data.
  * @throws CSException if <tt>parameterId</tt> is not a valid parameter.
  */
  public void setParam(int parameterId
                      ,Blob parameterValue) throws CSException
    {
    checkRange(parameterId);
    parameterArray[parameterId-1] = parameterValue;
    inputParameterSetArray[parameterId-1] = true;
    parameterTypeArray[parameterId-1] = Types.BLOB;
    }
          ...
          try
            ...
            if (   parameterArray[i]  == null)
              ...
              if (parameterTypeArray[i] != Integer.MIN_VALUE)
                {
               if ( parameterTypeArray[i] == Types.STRUCT)
                  {
              	  thePreparedStatement.setNull(i+1
              			  , Types.STRUCT
            ...
            else if (parameterArray[i] instanceof Clob)
              {
              thePreparedStatement.setClob(i+1, (Clob)parameterArray[i]);
              }
            else if (parameterArray[i] instanceof Blob)
              {
              thePreparedStatement.setBlob(i+1, (Blob)parameterArray[i]);
              }
            ...
            else if (parameterArray[i] instanceof JavaArrayObject)
              {
              java.sql.Array anArray = ((JavaArrayObject)parameterArray[i]).getArray(thePreparedStatement.getConnection());
              thePreparedStatement.setArray(i+1, anArray );
              }
            else
              {
